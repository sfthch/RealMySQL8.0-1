...
4.1.1 MySQL의 전체 구조
  ...
  | 프로그래밍 API                                               |
  |                                                              |
  | Java RulPh D.NET python                                      |
  |                                                              |
  | 커넥션 핸들러                                                |
  |                                                              |
  | SOL인터페이스   SQL 파서    SOL옵티마이저       캐시 & 버퍼  |
  |                                                              |
  | MySQL 엔진      MySQL 서버  스토리지 엔진 API  스토리지 엔진 |
  |                                                              |
  | InnoDB          MYISAM      Memory  디스크                   |
  |                                                              |
  | 데이터파일  로그파일    운영체제하드웨어                     |
  +--------------------------------------------------------------+
...
4.1.4 플러그인 스토리지 엔진 모델
  ...
  +---------------------------------------------------------------------------------+
  |                                  MySQL 서버                                     |
  |     InnoDB      validate_password   query rewrite   Transparent Data Encryption |
  | 스토리지 엔진     컴포넌트            플러그인              플러그인            |
  +---------------------------------------------------------------------------------+
  ...
  +--------------------------------------------------------------------------+
  | SQL 파서  SQL옵티마이저   SQL 실행기      데이터읽기/쓰기                |
...
4.1.9 스레드 풀
  ...
  +----------------------------------------------+
  | COMMIT -> COMMIT -> COMMIT -> QUERY -> QUERY |
  | BEGIN <- BEGIN <- BEGIN <- QUERY <------+    |
  +----------------------------------------------+
  그림 4.7 사용자로부터 요청이 유입된 순서

  +----------------------------------------------+
  | COMMIT -> QUERY -> COMMIT -> QUERY -> COMMIT |
  | BEGIN <- BEGIN <- BEGIN <- QUERY <------+    |
  +----------------------------------------------+
...
4.2 InnoDB 스토리지 엔진 아키텍처
  ...
  |                체인지 버퍼                                          |
  |  SOL실행기                           백그라운드 스레드              |
  |  체인지 버퍼     데이터 페이지   로그기록        디스크             |
  |  머지 스레드     기록 스레드      스레드                            |
  |                                                                     |
  |  데이터 파일     트랜잭션 로그 파일      디스크                     |
  +---------------------------------------------------------------------+
  그림 4.9 InnoDB 구조
...
4.2.3 MVCC(Multi Version Concurrency Control)
  ...
  |                   메모리                        |
  | InnoDB 버퍼 풀                     언두 로그    |
  | member m_id m_name m_area                       |
  |         12  홍길동 서울    데이터 파일 (디스크) |
  |         m_id  m_name  m_area                    |
  |          12   홍길동  서울                      |
  +-------------------------------------------------+
  ...
  |                   메모리                                                    |
  |       InnoDB 버퍼 풀                  언두 로그                             |
  | member  m_id m_name m_area    m_id m_area                                   |
  |          12  홍길동 경기       12  서울                                     |
  |                  m_area 칼럼의 변경 전                                      |
  |                  값만 언두 로그로 복사                                      |
  |                                      데이터 파일 (디스크)                   |
  | member     m_id m_name m_area                                               |
  |             12  홍길동      ?                                               |
  |    InnoDB 버퍼 풀의 변경 내용은 InnoDB 스토리지 엔진의                      |
  |    백그라운드 스레드에 의해서 기록된다. 그래서 InnoDB 버퍼 품의 변경 내용이 |
  |    디스크의 데이터 파일에 기록됐는지 여부는 시점에 따라 다를 수 있다.       |
  +-----------------------------------------------------------------------------+
...
4.2.8 Double Write Buffer
  ...
  | InnoDB 버퍼 풀                                             |
  |       CA                                                   |
  |               1. 변경된 데이터 페이지를 모아서 한번에 기록 |
  |               2. 데이터 페이지를 개별로 파일로 기록        |
  | DABCDC                                                     |
  | 데이터 파일   DoubleWrite HD                               |
  |              (시스템 테이블스페이스)                       |
  +------------------------------------------------------------+
...
4.2.12 어댑티브 해시 인덱스
  ...
  +-------------------------------+
  | 100 80 60 CPU Usage (%) 40 20 |
  +-------------------------------+
  그림 4.18 CPU 사용률 변화

  +-------------------------------------------------+
  | 50000 40000 30000 SELECT / second / 20000 10000 |
  +-------------------------------------------------+
...
4.2.13 InnoDB와 MyISAM, MEMORY 스토리지 엔진 비교
  ...
  지금도 가끔씩 MyISAM이나 MEMORY 스토리지 엔진에 대한 성능상 장점을 기대하는 사용자들이 1는데,
